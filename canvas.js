...
var magnify = false;
canvasHover.addEventListener("mousedown", () => { magnify = true });
canvasHover.addEventListener("mouseup", () => { magnify = false });
...
function distFromMouse(v) {
    return Math.sqrt(
        Math.pow(v.x - mouse.x, 2) + Math.pow(v.y - mouse.y, 2)
    );
}
...
function animateHover() {
    ...
    if (magnify) {
        ...
        const hoverArray = pointsArray.filter(p => distFromMouse(p) < magnifyRadius);
        hoverArray.forEach(
            p => drawPoint(
                mouse.x + (p.x - mouse.x) * 100 / magnifyRadius,
                mouse.y + (p.y - mouse.y) * 100 / magnifyRadius, 1, ctxHover, "black")
        )
        ...
        if (mouse.x) {
            const hoverArray = pointsArray.filter(p => distFromMouse(p) < magnifyRadius);
            hoverArray.forEach(
                p => drawPoint(p.x, p.y, 1, ctxHover, "green")
            )
        }
    }
    ...
    ctxHover.fillText(`[${vertexArray[0].x}, ${Math.round(vertexArray[0].y)}]`, canvasHover.width - 120, 20);
    ctxHover.fillText(`[${vertexArray[1].x}, ${Math.round(vertexArray[1].y)}]`, canvasHover.width - 120, 40);
    ctxHover.fillText(`[${vertexArray[2].x}, ${Math.round(vertexArray[2].y)}]`, canvasHover.width - 120, 60);
}
